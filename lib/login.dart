...
Future<void> signInWithKakao() async {
  if (await AuthApi.instance.hasToken()) {
    try {
      ...
      _logger.i('토큰 유효성 체크 성공: ${tokenInfo.id} ${tokenInfo.expiresIn}');
      _getUserInfo();
    } catch (error) {
      if (error is KakaoException && error.isInvalidTokenError()) {
        _logger.w('토큰 만료: $error');
      } else {
        _logger.e('토큰 정보 조회 실패: $error');
      }
...
Future<void> loginWithKakaoAccount() async {
  if (await isKakaoTalkInstalled()) {
    try {
      ...
      _logger.i('카카오톡으로 로그인 성공: ${token.accessToken}');
      _getUserInfo();
    } catch (error) {
